...
neededPackages <- c("leaflet", "raster", "mapview", "sp", "ggmap",
                    "rasterVis",   "ggplot2", "dplyr", "RgoogleMaps", "rworldmap", "lubridate", "zoo", "broom", "caret", "gbm", "caTools", "ROCR", "pdp", "cowplot")
...
df_Ornithodoros = df
save(df_Ornithodoros, file = "df_Ornithodoros.Rdata")
```
...
mp<- ggplot()+world
    ...
    mp <- mp+
   geom_point(data = df, aes(x = decimalLongitude, y = decimalLatitude, color = species))
  ggsave("Ornithodoros_points.jpg", plot = mp, height = 7, width = 7*2)
...
#https://stackoverflow.com/questions/35580730/raster-of-latitudes-and-longitudes/35592230
...
r_lat_resampled = resample(x = lat, y = w)
names(r_lat_resampled)="latitude"

w = addLayer(w, r_lat_resampled)
...
#use (worldclim, latitude, and ecoregion) predictors to make predictions about Ornithodoros
```{r orn_preds}
library(dismo)
...
pdf("myOut_percentile_latitude.pdf")
...
#  p_percentile <- calc(p, fun = quantile(probs = seq(from = 0, to = 1, by = 0.01)))
  p_points = rasterToPoints(p, spatial=TRUE)#convert to Points
  p_points$layer_percentile = percent_rank(p_points$layer)#calc percent_rank using dplyr
  p_raster = rasterize(x = p_points, y = p, field = "layer_percentile")#rasterize 
  p = p_raster#now go back and reassign p_raster to p
...
x.df$variable_plot[x.df$variable_plot=="countrGeorgia"]="Georgia"
x.df$variable_plot[x.df$variable_plot=="countryBelarus"]="Belarus"
...
###partial dependency -- alternative method -- ticks only
```{r}

#source("partial_dependency_ticks.R")
```

###partial dependency -- alternative method -- all else
```{r}

#source("partial_dependency_all_else.R")
```


##PARTIAL dependency -- JP Schmidt method w/ adjustments -- non-ticks
```{r}
...
ls<-partial(gbmtest,n.trees=best.iter, "log_pigs",prob=TRUE)

plot1 <- ls %>%
  select(log_pigs, yhat) %>%
  na.omit() %>%
  ggplot() +
  geom_line(aes(x = log_pigs, y = yhat),color="red") +
  # ylim(0.,1) +
  ylab(" ") +
  theme_minimal() +
...
p1 <- add_sub(plot1," ")
p2 <- add_sub(plot2,"log (pigs)",size=12)
...
###chickens
lc<-partial(gbmtest,n.trees=best.iter, "log_chickens",prob=TRUE)

plot1 <- lc %>%
  select(log_chickens, yhat) %>%
  na.omit() %>%
  ggplot() +
  geom_line(aes(x = log_chickens, y = yhat),color="red") +
  # ylim(0.,1) +
  ylab(" ") +
  theme_minimal() +
  theme(axis.title.x = element_blank())+ theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
...
p1 <- add_sub(plot1," ")
p2 <- add_sub(plot2,"log (chickens)",size=12)
aligned <- align_plots(p1, p2, align = "v")
plc <- ggdraw()+
  draw_plot(aligned[[2]])+
  draw_plot(aligned[[1]],
...
lca<-partial(gbmtest,n.trees=best.iter, "log_cattle",prob=TRUE)

plot1 <- lca %>%
  select(log_cattle, yhat) %>%
  na.omit() %>%
  ggplot() +
  geom_line(aes(x = log_cattle, y = yhat),color="red") +
  # ylim(0.,1) +
  ylab(" ") +
  theme_minimal() +
  theme(axis.title.x = element_blank())+ theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
...
p1 <- add_sub(plot1," ")
p2 <- add_sub(plot2,"log (cattle)",size=12)
aligned <- align_plots(p1, p2, align = "v")
plca <- ggdraw()+
  draw_plot(aligned[[2]])+
  draw_plot(aligned[[1]],
...
###African warthog
caf<-partial(gbmtest,n.trees=best.iter, "Phacochoerus_africanus",prob=TRUE)

plot1 <- caf %>%
  select(Phacochoerus_africanus, yhat) %>%
  na.omit() %>%
  ggplot() +
  geom_line(aes(x = Phacochoerus_africanus, y = yhat),color="red") +
  # ylim(0.,1) +
  ylab(" ") +
  theme_minimal() +
  theme(axis.title.x = element_blank())+ theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
...
plot2 <- Train %>%
  select(Phacochoerus_africanus) %>%
  na.omit() %>%
  ggplot(aes(Phacochoerus_africanus)) +
  geom_histogram(fill="lightblue") +
  scale_y_continuous(position="right",breaks = function(x) unique(floor(pretty(seq(0, (max(x) + 1) * 1.1)))))+
  ylab(" ") +
  theme_minimal() +
...
p1 <- add_sub(plot1," ")
p2 <- add_sub(plot2,"Phacochoerus africanus range",size=12)
aligned <- align_plots(p1, p2, align = "v")
pcaf <- ggdraw()+
  draw_plot(aligned[[2]])+
  draw_plot(aligned[[1]],
...
###nightlights
caf<-partial(gbmtest,n.trees=best.iter, "nightlights2013",prob=TRUE)

plot1 <- caf %>%
  select(nightlights2013, yhat) %>%
  na.omit() %>%
  ggplot() +
  geom_line(aes(x = nightlights2013, y = yhat),color="red") +
  # ylim(0.,1) +
  ylab(" ") +
  theme_minimal() +
  theme(axis.title.x = element_blank())+ theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
...
plot2 <- Train %>%
  select(nightlights2013) %>%
  na.omit() %>%
  ggplot(aes(nightlights2013)) +
  geom_histogram(fill="lightblue") +
  scale_y_continuous(position="right",breaks = function(x) unique(floor(pretty(seq(0, (max(x) + 1) * 1.1)))))+
  ylab(" ") +
  theme_minimal() +
...
p1 <- add_sub(plot1," ")
p2 <- add_sub(plot2,"nightlights",size=12)
aligned <- align_plots(p1, p2, align = "v")
pnt <- ggdraw()+
  draw_plot(aligned[[2]])+
  draw_plot(aligned[[1]],
...
#Sus scrofa
caf<-partial(gbmtest,n.trees=best.iter, "Sus_scrofa",prob=TRUE)

plot1 <- caf %>%
  select(Sus_scrofa, yhat) %>%
  na.omit() %>%
  ggplot() +
  geom_line(aes(x = Sus_scrofa, y = yhat),color="red") +
  # ylim(0.,1) +
  ylab(" ") +
  theme_minimal() +
  theme(axis.title.x = element_blank())+ theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
...
p1 <- add_sub(plot1," ")
p2 <- add_sub(plot2,"Sus scrofa range",size=12)
aligned <- align_plots(p1, p2, align = "v")
pss <- ggdraw()+
  draw_plot(aligned[[2]])+
  draw_plot(aligned[[1]],
...
##sheep
lca<-partial(gbmtest,n.trees=best.iter, "log_sheep",prob=TRUE)

plot1 <- lca %>%
  select(log_sheep, yhat) %>%
  na.omit() %>%
  ggplot() +
  geom_line(aes(x = log_sheep, y = yhat),color="red") +
  # ylim(0.,1) +
  ylab(" ") +
  theme_minimal() +
  theme(axis.title.x = element_blank())+ theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
...
p1 <- add_sub(plot1," ")
p2 <- add_sub(plot2,"log (sheep)",size=12)
aligned <- align_plots(p1, p2, align = "v")
pllsh <- ggdraw()+
  draw_plot(aligned[[2]])+
  draw_plot(aligned[[1]],
...
#log_road_pigs
lca<-partial(gbmtest,n.trees=best.iter, "log_road_pigs",prob=TRUE)

plot1 <- lca %>%
  select(log_road_pigs, yhat) %>%
  na.omit() %>%
  ggplot() +
  geom_line(aes(x = log_road_pigs, y = yhat),color="red") +
  # ylim(0.,1) +
  ylab(" ") +
  theme_minimal() +
  theme(axis.title.x = element_blank())+ theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
...
plot2 <- Train %>%
  select(log_road_pigs) %>%
  na.omit() %>%
  ggplot(aes(log_road_pigs)) +
  geom_histogram(fill="lightblue") +
  scale_y_continuous(position="right",breaks = function(x) unique(floor(pretty(seq(0, (max(x) + 1) * 1.1)))))+
  ylab(" ") +
  theme_minimal() +
...
p1 <- add_sub(plot1," ")
p2 <- add_sub(plot2,"log (roads X pigs)",size=12)
aligned <- align_plots(p1, p2, align = "v")
plrp <- ggdraw()+
  draw_plot(aligned[[2]])+
  draw_plot(aligned[[1]],
...
lca<-partial(gbmtest,n.trees=best.iter, "log_road_sheep",prob=TRUE)

plot1 <- lca %>%
  select(log_road_sheep, yhat) %>%
  na.omit() %>%
  ggplot() +
  geom_line(aes(x = log_road_sheep, y = yhat),color="red") +
  # ylim(0.,1) +
  ylab(" ") +
  theme_minimal() +
  theme(axis.title.x = element_blank())+ theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
...
plot2 <- Train %>%
  select(log_road_sheep) %>%
  na.omit() %>%
  ggplot(aes(log_road_sheep)) +
  geom_histogram(fill="lightblue") +
  scale_y_continuous(position="right",breaks = function(x) unique(floor(pretty(seq(0, (max(x) + 1) * 1.1)))))+
  ylab(" ") +
  theme_minimal() +
...
p1 <- add_sub(plot1," ")
p2 <- add_sub(plot2,"log (roads X sheep)",size=12)
aligned <- align_plots(p1, p2, align = "v")
prshp <- ggdraw()+
  draw_plot(aligned[[2]])+
  draw_plot(aligned[[1]],
...
#georgia
lca<-partial(gbmtest,n.trees=best.iter, "countryGeorgia",prob=TRUE)

plot1 <- lca %>%
  select(countryGeorgia, yhat) %>%
  na.omit() %>%
  ggplot() +
  geom_line(aes(x = countryGeorgia, y = yhat),color="red") +
  # ylim(0.,1) +
  ylab(" ") +
  theme_minimal() +
  theme(axis.title.x = element_blank())+ theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
...
plot2 <- Train %>%
  select(countryGeorgia) %>%
  na.omit() %>%
  ggplot(aes(countryGeorgia)) +
  geom_histogram(fill="lightblue") +
  scale_y_continuous(position="right",breaks = function(x) unique(floor(pretty(seq(0, (max(x) + 1) * 1.1)))))+
  ylab(" ") +
  theme_minimal() +
...
p1 <- add_sub(plot1," ")
p2 <- add_sub(plot2,"Georgia",size=12)
aligned <- align_plots(p1, p2, align = "v")
plge <- ggdraw()+
  draw_plot(aligned[[2]])+
  draw_plot(aligned[[1]],
...
plot1 <- lca %>%
  select(mammalDiversity, yhat) %>%
  na.omit() %>%
  ggplot() +
  geom_line(aes(x = mammalDiversity, y = yhat),color="red") +
  # ylim(0.,1) +
  ylab(" ") +
  theme_minimal() +
  theme(axis.title.x = element_blank())+ theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
...
plot2 <- Train %>%
  select(mammalDiversity) %>%
  na.omit() %>%
  ggplot(aes(mammalDiversity)) +
  geom_histogram(fill="lightblue") +
  scale_y_continuous(position="right",breaks = function(x) unique(floor(pretty(seq(0, (max(x) + 1) * 1.1)))))+
  ylab(" ") +
  theme_minimal() +
...
p1 <- add_sub(plot1," ")
p2 <- add_sub(plot2,"mammal diversity",size=12)
aligned <- align_plots(p1, p2, align = "v")
pmd <- ggdraw()+
  draw_plot(aligned[[2]])+
  draw_plot(aligned[[1]],
...
#goats
lca<-partial(gbmtest,n.trees=best.iter, "log_goats",prob=TRUE)

plot1 <- lca %>%
  select(log_goats, yhat) %>%
  na.omit() %>%
  ggplot() +
  geom_line(aes(x = log_goats, y = yhat),color="red") +
  # ylim(0.,1) +
  ylab(" ") +
  theme_minimal() +
  theme(axis.title.x = element_blank())+ theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
...
p1 <- add_sub(plot1," ")
p2 <- add_sub(plot2,"log (goats)",size=12)
aligned <- align_plots(p1, p2, align = "v")
pg <- ggdraw()+
  draw_plot(aligned[[2]])+
  draw_plot(aligned[[1]],
...
lca<-partial(gbmtest,n.trees=best.iter, "log_road_chickens",prob=TRUE)

plot1 <- lca %>%
  select(log_road_chickens, yhat) %>%
  na.omit() %>%
  ggplot() +
  geom_line(aes(x = log_road_chickens, y = yhat),color="red") +
  # ylim(0.,1) +
  ylab(" ") +
  theme_minimal() +
  theme(axis.title.x = element_blank())+ theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
...
plot2 <- Train %>%
  select(log_road_chickens) %>%
  na.omit() %>%
  ggplot(aes(log_road_chickens)) +
  geom_histogram(fill="lightblue") +
  scale_y_continuous(position="right",breaks = function(x) unique(floor(pretty(seq(0, (max(x) + 1) * 1.1)))))+
  ylab(" ") +
  theme_minimal() +
...
p1 <- add_sub(plot1," ")
p2 <- add_sub(plot2,"log (roads X chickens)",size=12)
aligned <- align_plots(p1, p2, align = "v")
prc <- ggdraw()+
  draw_plot(aligned[[2]])+
  draw_plot(aligned[[1]],
...
###partial dependence -- JP Schmidt method w/ adjustments -- ticks
```{r}
scale = 0.98
load("TrainLivestock.Rdata")
load("gbmtestLivestock.Rdata")
...
plot1 <- ls %>%
  select(Carioserraticus, yhat) %>%
  na.omit() %>%
  ggplot() +
  geom_line(aes(x = Carioserraticus, y = yhat),color="red") +
  # ylim(0.,1) +
  ylab(" ") +
  theme_minimal() +
  theme(axis.title.x = element_blank())+ theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
...
plot2 <- Train %>%
  select(Carioserraticus) %>%
  na.omit() %>%
  ggplot(aes(Carioserraticus)) +
  geom_histogram(fill="lightblue") +
  scale_y_continuous(position="right",breaks = function(x) unique(floor(pretty(seq(0, (max(x) + 1) * 1.1)))))+
  ylab(" ") +
  theme_minimal() +
...
p1 <- add_sub(plot1," ")
p2 <- add_sub(plot2,"Carios erraticus",size=12)
...
#nicollei
ls<-partial(gbmtest,n.trees=best.iter, "Ornithodorosnicollei",prob=TRUE)

plot1 <- ls %>%
  select(Ornithodorosnicollei, yhat) %>%
  na.omit() %>%
  ggplot() +
  geom_line(aes(x = Ornithodorosnicollei, y = yhat),color="red") +
  # ylim(0.,1) +
  ylab(" ") +
  theme_minimal() +
  theme(axis.title.x = element_blank())+ theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
...
plot2 <- Train %>%
  select(Ornithodorosnicollei) %>%
  na.omit() %>%
  ggplot(aes(Ornithodorosnicollei)) +
  geom_histogram(fill="lightblue") +
  scale_y_continuous(position="right",breaks = function(x) unique(floor(pretty(seq(0, (max(x) + 1) * 1.1)))))+
  ylab(" ") +
  theme_minimal() +
...
p1 <- add_sub(plot1," ")
p2 <- add_sub(plot2,"Ornithodoros nicollei",size=12)
...
#gurneyi
ls<-partial(gbmtest,n.trees=best.iter, "Ornithodorosgurneyi",prob=TRUE)

plot1 <- ls %>%
  select(Ornithodorosgurneyi, yhat) %>%
  na.omit() %>%
  ggplot() +
  geom_line(aes(x = Ornithodorosgurneyi, y = yhat),color="red") +
  # ylim(0.,1) +
  ylab(" ") +
  theme_minimal() +
  theme(axis.title.x = element_blank())+ theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
...
plot2 <- Train %>%
  select(Ornithodorosgurneyi) %>%
  na.omit() %>%
  ggplot(aes(Ornithodorosgurneyi)) +
  geom_histogram(fill="lightblue") +
  scale_y_continuous(position="right",breaks = function(x) unique(floor(pretty(seq(0, (max(x) + 1) * 1.1)))))+
  ylab(" ") +
  theme_minimal() +
...
p1 <- add_sub(plot1," ")
p2 <- add_sub(plot2,"O. gurneyi",size=12)
...
#moubata
ls<-partial(gbmtest,n.trees=best.iter, "Ornithodorosmoubata",prob=TRUE)

plot1 <- ls %>%
  select(Ornithodorosmoubata, yhat) %>%
  na.omit() %>%
  ggplot() +
  geom_line(aes(x = Ornithodorosmoubata, y = yhat),color="red") +
  # ylim(0.,1) +
  ylab(" ") +
  theme_minimal() +
  theme(axis.title.x = element_blank())+ theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
...
plot2 <- Train %>%
  select(Ornithodorosmoubata) %>%
  na.omit() %>%
  ggplot(aes(Ornithodorosmoubata)) +
  geom_histogram(fill="lightblue") +
  scale_y_continuous(position="right",breaks = function(x) unique(floor(pretty(seq(0, (max(x) + 1) * 1.1)))))+
  ylab(" ") +
  theme_minimal() +
...
p1 <- add_sub(plot1," ")
p2 <- add_sub(plot2,"O. moubata",size=12)
...
#tholozani
ls<-partial(gbmtest,n.trees=best.iter, "Ornithodorostholozani",prob=TRUE)

plot1 <- ls %>%
  select(Ornithodorostholozani, yhat) %>%
  na.omit() %>%
  ggplot() +
  geom_line(aes(x = Ornithodorostholozani, y = yhat),color="red") +
  # ylim(0.,1) +
  ylab(" ") +
  theme_minimal() +
  theme(axis.title.x = element_blank())+ theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
...
plot2 <- Train %>%
  select(Ornithodorostholozani) %>%
  na.omit() %>%
  ggplot(aes(Ornithodorostholozani)) +
  geom_histogram(fill="lightblue") +
  scale_y_continuous(position="right",breaks = function(x) unique(floor(pretty(seq(0, (max(x) + 1) * 1.1)))))+
  ylab(" ") +
  theme_minimal() +
...
p1 <- add_sub(plot1," ")
p2 <- add_sub(plot2,"O. tholozani",size=12)
...
#macmillani
ls<-partial(gbmtest,n.trees=best.iter, "Ornithodorosmacmillani",prob=TRUE)

plot1 <- ls %>%
  select(Ornithodorosmacmillani, yhat) %>%
  na.omit() %>%
  ggplot() +
  geom_line(aes(x = Ornithodorosmacmillani, y = yhat),color="red") +
  # ylim(0.,1) +
  ylab(" ") +
  theme_minimal() +
  theme(axis.title.x = element_blank())+ theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
...
plot2 <- Train %>%
  select(Ornithodorosmacmillani) %>%
  na.omit() %>%
  ggplot(aes(Ornithodorosmacmillani)) +
  geom_histogram(fill="lightblue") +
  scale_y_continuous(position="right",breaks = function(x) unique(floor(pretty(seq(0, (max(x) + 1) * 1.1)))))+
  ylab(" ") +
  theme_minimal() +
...
p1 <- add_sub(plot1," ")
p2 <- add_sub(plot2,"O. macmillani",size=12)
...
#turicata
ls<-partial(gbmtest,n.trees=best.iter, "Ornithodorosturicata",prob=TRUE)

plot1 <- ls %>%
  select(Ornithodorosturicata, yhat) %>%
  na.omit() %>%
  ggplot() +
  geom_line(aes(x = Ornithodorosturicata, y = yhat),color="red") +
  # ylim(0.,1) +
  ylab(" ") +
  theme_minimal() +
  theme(axis.title.x = element_blank())+ theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
...
plot2 <- Train %>%
  select(Ornithodorosturicata) %>%
  na.omit() %>%
  ggplot(aes(Ornithodorosturicata)) +
  geom_histogram(fill="lightblue") +
  scale_y_continuous(position="right",breaks = function(x) unique(floor(pretty(seq(0, (max(x) + 1) * 1.1)))))+
  ylab(" ") +
  theme_minimal() +
...
p1 <- add_sub(plot1," ")
p2 <- add_sub(plot2,"O. turicata",size=12)
...
#hermsi
ls<-partial(gbmtest,n.trees=best.iter, "Ornithodoroshermsi",prob=TRUE)

plot1 <- ls %>%
  select(Ornithodoroshermsi, yhat) %>%
  na.omit() %>%
  ggplot() +
  geom_line(aes(x = Ornithodoroshermsi, y = yhat),color="red") +
  # ylim(0.,1) +
  ylab(" ") +
  theme_minimal() +
  theme(axis.title.x = element_blank())+ theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
...
plot2 <- Train %>%
  select(Ornithodoroshermsi) %>%
  na.omit() %>%
  ggplot(aes(Ornithodoroshermsi)) +
  geom_histogram(fill="lightblue") +
  scale_y_continuous(position="right",breaks = function(x) unique(floor(pretty(seq(0, (max(x) + 1) * 1.1)))))+
  ylab(" ") +
  theme_minimal() +
...
p1 <- add_sub(plot1," ")
p2 <- add_sub(plot2,"O. hermsi",size=12)
...
#savignyi
ls<-partial(gbmtest,n.trees=best.iter, "Ornithodorossavignyi",prob=TRUE)

plot1 <- ls %>%
  select(Ornithodorossavignyi, yhat) %>%
  na.omit() %>%
  ggplot() +
  geom_line(aes(x = Ornithodorossavignyi, y = yhat),color="red") +
  # ylim(0.,1) +
  ylab(" ") +
  theme_minimal() +
  theme(axis.title.x = element_blank())+ theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
...
plot2 <- Train %>%
  select(Ornithodorossavignyi) %>%
  na.omit() %>%
  ggplot(aes(Ornithodorossavignyi)) +
  geom_histogram(fill="lightblue") +
  scale_y_continuous(position="right",breaks = function(x) unique(floor(pretty(seq(0, (max(x) + 1) * 1.1)))))+
  ylab(" ") +
  theme_minimal() +
...
p1 <- add_sub(plot1," ")
p2 <- add_sub(plot2,"O. savignyi",size=12)
...
mp<- ggplot()+world
  ...
  #ggsave(filename = "predicted.jpg",plot = mp)  
...
#surveillance priorities with hex plot and geom_point
```{r surveillance}
...
mp <- mp+ geom_point(data = df, aes(x=x, y=y),
                     color = 'red',
                     alpha = 0.05)+
theme(legend.position="bottom")+
  guides(fill=guide_legend(title="Locs. w/ probability>0.9, >=1 deg. from African swine fever in pigs"))+
...
mp
ggsave(plot = mp, filename = "points_90percent_predicted_over_1deg_outbreak_swine_swine_no_hex.jpg")
...
mp
ggsave(plot = mp, filename = "points_90percent_predicted_under_1deg_outbreak_swine_swine_no_hex.jpg")
